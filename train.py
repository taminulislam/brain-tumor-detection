...
import config
from model import get_efficient_mtnet_model
from datasets import get_dataloaders
from losses import MultiTaskLoss
from utils import (calculate_classification_metrics, calculate_iou,
...
class Trainer:
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            # Handle auxiliary heads if present
            if len(outputs) == 4:  # Training mode with auxiliary heads
                cls_output, seg_output, aux1, aux2 = outputs
                aux_preds = [aux1, aux2]
            else:  # Inference mode
            ...
            # Calculate loss
            loss_dict = self.criterion(cls_output, seg_output, cls_labels, seg_masks, aux_preds)
    ...
    def validate_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            for batch in pbar:
                ...
                # Handle auxiliary heads if present (but we only use main outputs for validation)
                if len(outputs) == 4:  # Training mode with auxiliary heads
                    cls_output, seg_output, _, _ = outputs
                else:  # Inference mode
                    cls_output, seg_output = outputs

                # Calculate loss (without auxiliary heads for validation)
                loss_dict = self.criterion(cls_output, seg_output, cls_labels, seg_masks, None)
                total_loss = loss_dict['total_loss']
